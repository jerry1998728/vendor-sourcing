...
Acceptance: all three configs run; github_org yields >= 15 candidates; a 5-row CSV lands with correct badges; every filter works; queue actions write events; Reject without reason is blocked; adding a fourth web-search config needs no code change. Stop and report.
