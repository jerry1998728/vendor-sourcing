...
2. Check Evaluate → Draft & Send: the **Sending** button should read "Gmail connected · 1 allowed". If not: `.env.local` needs `DEMO_ALLOWED_RECIPIENTS`, and **Connect Gmail** writes `token.json`.
...
**2. Discovery, 90 seconds.** Discover → Vendor Data. Pick `ego_data_stereo` in the config selector, press **Run config**. While it runs (five candidates, haiku), open Discover → Vendor Source and show the channel selector: type a requirement, **Generate seed queries** proposes eight (each one is a web search the run executes), edit them, **Save config & run** writes a YAML config, no code. Show the GitHub card: languages are a checkbox picker, and the **+** next to any ruleset clones it into a new validated ruleset file. When the run finishes the badges show discovered, vendor sites, pass / unknown / fail, coverage and unknown rate; the **Runs** button in the header shows the `ruleset_version`.
...
**4. Review Queue (Select), 60 seconds.** One vendor at a time, unknown must-fields pinned on top. **Qualify** Verity POV, **Reject** Helix and show the reason selector is required, **Need info** on an unknown vendor: it becomes Qualified with `outreach_to_verify`.

**5. Draft & Send, 90 seconds.** Evaluate → Draft & Send → **WearCapture** → **Generate draft**. Read the draft: it cites the stereo glasses and Austin registration and asks who owns the company. Set the recipient to your hotmail address, edit a line, **Send**. Vendor becomes Contacted with the Gmail thread id; open its page to show the human event on the Timeline.

**6. Reply and inference, 90 seconds.** In hotmail, reply to the email with "Happy to talk, can we set up a call next week?". Evaluate → Proposals → **Poll inbox**: Contacted → Replied is a system event, then inference applies In Discussion / responded at 0.9 confidence. Click the vendor name: the Timeline is a vertical rail (the sent email, the reply, the amber inference milestone), press **Revert** on the inference event, show it reversed with a human event, then **Back** returns to Proposals.
...
**6b. Diligence, 45 seconds.** On the same vendor open the **Diligence** tab: the checklist comes from its ruleset, grouped by technical, security, compliance and financial. Answer **Pricing received in writing** with the quote's URL: it counts, the tab header moves to 1 of 8, and the Timeline records who checked it against which source. Answer one without a URL to show it stays unverified and does not count. The stage selector on the right moves the vendor through technical_review, quote_received and sampling.

**7. Board and follow-ups, 45 seconds.** Manage → Board: columns by status, an overdue due date in red, the diligence stage on In Discussion cards. Proposals → **Run follow-ups**: silent vendors get a 7-day draft and become Dormant after 10 days (Alpine Egocentric already shows this).

**8. Refresh, 30 seconds, optional.** Vendor Source → **Schedules** → **Refresh now** on `code_data_github_orgs` (GitHub API only, free). Open a refreshed vendor's Timeline: `tag_changed` with the diff, and the schedule row now shows the last run.
